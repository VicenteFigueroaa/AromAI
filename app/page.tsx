      ...
      {result && result.data && (
        <div className="w-full max-w-md mt-6 bg-slate-800 rounded-2xl shadow-xl overflow-hidden border border-slate-700 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
          ...
          <div className={`w-full h-48 relative overflow-hidden flex flex-col items-center justify-center transition-all duration-300 ${isChangingVariant ? 'opacity-50 grayscale' : ''}`}>
            ...
            {/* Contenido Central: Ícono o Imagen del Perfume */}
            <div className="relative z-10 text-center">
              <div className="w-40 h-40 mx-auto bg-slate-800/80 backdrop-blur-md border border-slate-700/80 rounded-2xl flex items-center justify-center shadow-2xl mb-2 overflow-hidden">
                {result.data.image_url ? (
                  <img src={result.data.image_url} alt={result.data.name} className="w-full h-full object-cover" />
                ) : (
                  <svg className="w-20 h-20 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
